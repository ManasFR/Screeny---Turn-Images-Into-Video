...
approveUrl: orderData.links?.find((l: { rel: string; href: string }) => l.rel === 'approve')?.href,
        });
      }
...
return NextResponse.json({ error: (err as Error).message || 'Order creation failed' }, { status: 500 });
  }
}
