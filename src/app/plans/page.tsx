...
declare global {
  interface Window {
    ...
    paypal: { Buttons: (options: Record<string, unknown>) => { render: (id: string) => void } };
  }
}
...
const PaymentModal = ({ plan, onClose, userEmail, userName }: PaymentModalProps) => {
  ...
  const handleRazorpay = useCallback(async () => {
    ...
    try {
      ...
      setLoading(null);
      setErrorMsg((err as Error).message || 'Razorpay failed to load');
      setStep('error');
    }
  ...
  const handlePayPal = useCallback(async () => {
    ...
    try {
      ...
      setLoading(null);
      setErrorMsg((err as Error).message || 'PayPal failed to load');
      setStep('error');
    }
  }, [plan, paypalRendered]);
